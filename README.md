...
**Two-phase restore:** The restore step runs in two phases. Phase 1 checks the marker under a per-*target* lock — keyed by the restore path, not the cache key, so runners restoring to their own paths never contend and steady-state restores stay parallel across the fleet. If the target is already current, the restore finishes there. Otherwise Phase 2 takes the per-*key* lock (shared with the save step via [`curlewlabs-com/local-mutex`](https://github.com/curlewlabs-com/local-mutex)) and, nested inside it, that same per-target lock, then performs the full rsync. The per-target lock is what lets the [`gc`](#the-gc-action) reclaim a restore target without racing a restore to it; the per-key lock serializes cold-start restores against each other and against concurrent saves of the same key. No actor holds the target lock while waiting on the key lock (Phase 1 takes the target lock alone; Phase 2 and the gc nest key ⊃ target), so a restore and a sweep can never deadlock.
