...
privately via
[GitHub's security advisory feature](https://github.com/curlewlabs-com/local-cache/security/advisories/new).
...
- **Key-to-path mapping:** The `key` and `restore-keys` inputs are hashed
  ...
  This blocks path traversal via cache keys without conflating distinct raw
  keys such as `a/b` and `a:b`.
